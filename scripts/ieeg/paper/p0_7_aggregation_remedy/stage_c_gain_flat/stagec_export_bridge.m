function report=stagec_export_bridge(root,outputRoot,mode)
%STAGEC_EXPORT_BRIDGE MATLAB-only reader for v7.3 tables; no statistics.
arguments
    root (1,1) string
...
toolDir=fileparts(mfilename("fullpath"));c=jsondecode(fileread(fullfile(toolDir,"stagec_mphase_gainflat_contract.json")));
...
pairParts=cell(numel(files),1);subjectParts=pairParts;worldParts=cell(numel(files)*4,1);np=0;ns=0;nw=0;
for i=1:numel(files)
    z=load(fullfile(files(i).folder,files(i).name),"checkpoint");q=z.checkpoint;
    assertIdentity(q,c,root,toolDir,mode);
    ar=q.arm_results;np=np+1;pr=ar.pair_rows;pr.seed_index(:)=q.seed_index;pr.world_seed(:)=q.world_seed;pr.mode(:)=string(q.mode);pr.arm(:)=string(ar.arm);pr.kappa(:)=ar.kappa;pr.gain_slope_b(:)=ar.gain_slope_b;
    pairParts{np}=pr(:,["seed_index" "world_seed" "mode" "arm" "kappa" "gain_slope_b" "subject" "run" "session_id" "pair_id" ...
    ...
    for reducer=["median" "mean"]
        for run=["R1" "R2"]
            ...
            worldParts{nw}=table(q.seed_index,q.world_seed,string(q.mode),string(ar.arm),ar.kappa,ar.gain_slope_b,reducer,run,double(sf.n_subjects),double(sf.observed(2)),double(sf.p_two_sided(2)),double(sf.studentization_sd(2)),double(d.subject_statistic_skewness),double(d.n_positive),double(d.n_negative),double(d.n_zero),double(d.positive_fraction),string(ar.metric_stream_initial_state_hash),string(q.base_stream_state_hash),double(q.truth.A_G06_uV),string(q.truth.grid_id),'VariableNames',["seed_index" "world_seed" "mode" "arm" "kappa" "gain_slope_b" "reducer" "run" "n_subjects" "observed" "p_two_sided" "studentization_sd" "subject_statistic_skewness" "n_positive" "n_negative" "n_zero" "positive_fraction" "metric_stream_initial_state_hash" "base_stream_state_hash" "truth_a_g06_uV" "truth_grid_id"]);
        end
    end
end
...
report=struct("status","PASS","mode",mode,"checkpoints",numel(files),"pair_rows",height(pairs),"subject_rows",height(subjects),"world_rows",height(worlds));
fid=fopen(fullfile(outputRoot,"bridge_report.json"),"w");assert(fid>=0);cl=onCleanup(@()fclose(fid));fprintf(fid,"%s\n",jsonencode(report,"PrettyPrint",true));fclose(fid);disp(jsonencode(report,"PrettyPrint",true));
end
function assertIdentity(q,c,root,toolDir,mode)
assert(string(q.analysis_id)==string(c.analysis_id)&&string(q.mode)==mode&&q.status=="complete"&&numel(q.arm_results)==1,"Bridge identity mismatch.");
assert(string(q.schedule_sha256)==string(c.schedule.sha256),"Bridge schedule mismatch.");
assert(string(q.contract_sha256)==sha256File(fullfile(toolDir,"stagec_mphase_gainflat_contract.json")),"Bridge contract hash mismatch.");
...
assert(string(q.arm_results.arm)=="P0F","Bridge arm mismatch.");
