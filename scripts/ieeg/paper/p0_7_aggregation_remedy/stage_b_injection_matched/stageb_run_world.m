function checkpoint = stageb_run_world(root, seedIndex, mode, outputRoot)
%STAGEB_RUN_WORLD Run one complete matched P0/P4 Stage B world.
arguments
    root (1,1) string
...
contractPath=fullfile(toolDir,"stageb_mphase_power_contract.json");
...
started=utcNow();clock=tic;world=stageb_generate_matched_world(root,c,mode,worldSeed);
arms=["P0" "P4"];armResults=repmat(armTemplate(),2,1);
for a=1:2
    arm=arms(a);pairs=world.pairs.(char(arm));metricStream=RandStream("Threefry","Seed",worldSeed);
    ...
    for i=1:numel(pairs)
        ...
        % Contract intersection_rule: phaseGood & mx is a subset of the
        % source_complete injection set and its wider filter support implies
    ...
    armResults(a)=struct("status","complete","arm",arm,"kappa",double(c.arms.(char(arm)).kappa), ...
...
assert(isequal(string({armResults.arm}),arms),"Matched arm order mismatch.");
assertBasePairing(armResults);
checkpoint=struct("status","complete","analysis_id",string(c.analysis_id),"mode",mode, ...
    "seed_index",seedIndex,"world_seed",worldSeed,"arms",arms, ...
    ...
    "generator_sha256",sha256File(fullfile(toolDir,"stageb_generate_matched_world.m")), ...
    "input_hashes",inputHashes(base.inputs),"truth",world.truth,"arm_results",armResults, ...
...
function q=armTemplate(),q=struct("status","","arm","","kappa",NaN,"pair_rows",table(),"subject_rows",table(),"inference",struct(),"run_diagnostics",table(),"metric_stream_initial_state_hash","","metric_stream_final_state_hash","");end
...
assert(c.injection.gain_intercept_a==0&&c.injection.gain_slope_b==0.3055533803,"Gain constants changed.");
...
seed=double(c.seed_namespace.base)+idx;for e=c.seed_namespace.excluded_seed_ranges',assert(seed<double(e.range(1))||seed>double(e.range(2)),"Seed overlap.");end
end
...
assert(isequal(string(q.arms),["P0" "P4"])&&numel(q.arm_results)==2&&all(string({q.arm_results.status})=="complete"),"Incomplete matched arms.");
...
assert(string(q.generator_sha256)==string(c.local_generator_sha256),"Stage-B generator hash mismatch.");
end
function assertBasePairing(a)
p0=a(1).pair_rows;p4=a(2).pair_rows;assert(height(p0)==height(p4),"Arm pair count mismatch.");
assert(isequal(p0(:,["subject" "run" "session_id" "pair_id" "base_fingerprint"]),p4(:,["subject" "run" "session_id" "pair_id" "base_fingerprint"])),"Base pairing mismatch.");
assert(string(a(1).metric_stream_initial_state_hash)==string(a(2).metric_stream_initial_state_hash),"Metric stream starts differ.");
end
