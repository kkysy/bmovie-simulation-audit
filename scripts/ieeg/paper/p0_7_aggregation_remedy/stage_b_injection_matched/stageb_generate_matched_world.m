function world = stageb_generate_matched_world(root, contract, mode, seed)
%STAGEB_GENERATE_MATCHED_WORLD Stage-B-local matched P0/P4 synthesis.
% Base synthesis occurs once. Phase injection has pair/arm-local streams.
arguments
    root (1,1) string
    contract (1,1) struct
...
template=pairTemplate();pairsP0=repmat(template,height(p),1);pairsP4=pairsP0;
...
contractSha=sha256File(fullfile(fileparts(mfilename("fullpath")),"stageb_mphase_power_contract.json"));
...
for i=1:height(p)
    ...
    [inj0,eps0,n0,d0,s0]=phaseInjection(n1000,source,contract,0,phi,seed,"P0",pairKey,contractSha);
    [inj4,eps4,n4,d4,s4]=phaseInjection(n1000,source,contract,4,phi,seed,"P4",pairKey,contractSha); %#ok<ASGLU>
    inj0=filtfilt(blp,alp,inj0);inj4=filtfilt(blp,alp,inj4);
    ...
    q.signal=background250+inj0(1:4:end);q.epsilon_event_rad=eps0;q.n_injected_events=n0;q.injection_draw_count=d0;q.injection_stream_seed=s0;pairsP0(i)=q;
    q.signal=background250+inj4(1:4:end);q.epsilon_event_rad=eps4;q.injection_draw_count=d4;q.injection_stream_seed=s4;pairsP4(i)=q;
    if mod(i,10)==0||i==height(p),peakMemory=max(peakMemory,currentMemoryBytes());end
end
world=struct("pairs",struct("P0",pairsP0,"P4",pairsP4),"covariates",covariates, ...
    ...
    "kappa_P0",0,"kappa_P4",4,"R4_squared",0.7457,"frequency_hz",6), ...
